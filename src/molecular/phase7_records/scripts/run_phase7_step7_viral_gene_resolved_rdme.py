...
LIVE_ROOT = PHASE7 / "live_runs" / "step7_viral_gene_resolved"
...
def virus_like_name(virus: str) -> str:
    return f"{virus}-like"


def species_prefix(virus: str) -> str:
    return f"V_{virus}"


def parameter_path(virus: str) -> Path:
    return PHASE2 / "parameters" / f"{virus}_parameters.csv"


def read_viral_cds_table(path: Path, virus: str, expected_count: int | None = None) -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    target = virus_like_name(virus)
    with path.open(newline="") as fh:
        for row in csv.DictReader(fh):
            if row["virus"] == target:
                rows.append(row)
    ...
    if expected_count is not None and len(rows) != expected_count:
        raise ValueError(f"Phase7 {virus} requires {expected_count} CDS from cds_table.csv, got {len(rows)}")
    return rows
...
def parse_args():
    ...
    ap.add_argument("--virus", choices=VIRUS_CHOICES, default="P1")
...
def species_names(cds_rows: list[dict[str, str]], prefix: str) -> list[str]:
    names = [f"{prefix}_Genome", f"{prefix}_virion", f"{prefix}_membrane_damage", f"{prefix}_breach", f"{prefix}_released", f"{prefix}_rep_event", f"{prefix}_assembly_event"]
    for row in cds_rows:
        locus = row["locus_tag"]
        names.extend([
            f"{prefix}_G_{locus}",
            ...
            f"{prefix}_RB_{locus}",
            f"{prefix}_P_{locus}",
            f"{prefix}_TX_{locus}_event",
            f"{prefix}_TL_{locus}_event",
        ])
    return names


def species_diffusion(name: str, prefix: str) -> float:
    if name in {f"{prefix}_membrane_damage", f"{prefix}_breach", f"{prefix}_rep_event", f"{prefix}_assembly_event"} or name.endswith("_event"):
        return 0.0
    if name == f"{prefix}_Genome" or name.startswith(f"{prefix}_G_") or name.startswith(f"{prefix}_TX_"):
        return 5e-14
    if name.startswith(f"{prefix}_RB_"):
        return 5e-13
    if name.startswith(f"{prefix}_R_") or name.startswith(f"{prefix}_P_"):
        return 1e-12
    return 1e-13


def make_initial_particles(cds_rows: list[dict[str, str]], initial_genomes: int, placement_seed: int, prefix: str) -> list[dict]:
    ...
    for genome_i in range(initial_genomes):
        ...
        cursor += 1
        rows.append({"rdme_species": f"{prefix}_Genome", "x": x, "y": y, "z": z, "particle_no": particle_no, "source": f"phase7_initial_genome_{genome_i+1}"})
        particle_no += 1
        for cds in cds_rows:
            x, y, z = (int(v) for v in coords[cursor])
            cursor += 1
            rows.append({"rdme_species": f"{prefix}_G_{cds['locus_tag']}", "x": x, "y": y, "z": z, "particle_no": particle_no, "source": f"phase7_initial_gene_template_{genome_i+1}"})
            particle_no += 1
    return rows


def freeze_config(args, params: dict[str, str], cds_rows: list[dict[str, str]], particles: list[dict], params_path: Path) -> dict:
    for directory in [INPUT_FREEZE, CONFIG_DIR, REPORTS, GENERATED]:
        directory.mkdir(parents=True, exist_ok=True)
    prefix = species_prefix(args.virus)
    inputs = [CYTOPLASM_MASK, MEMBRANE_MASK, EXTRACELLULAR_MASK, CDS_TABLE, SUMMARY_TABLE, params_path, Path(__file__).resolve()]
    ...
    write_csv(GENERATED / f"{args.runId}_{args.virus.lower()}_cds_table.csv", cds_rows, list(cds_rows[0].keys()))
    intervals = [int(v) for v in str(args.dnaIntervals).split(",")]
    if intervals != [32, 16, 8, 4]:
        raise ValueError("Phase7 viral runner is locked to adaptive DNA intervals 32,16,8,4")
    config = {
        "run_id": args.runId,
        "phase": "Phase7",
        "virus": args.virus,
        "virus_like": virus_like_name(args.virus),
        "species_prefix": prefix,
        "scope": f"Gene-resolved {args.virus}-in-Syn3A RDME/WCM: {len(cds_rows)} {args.virus} CDS, independent mRNA/protein/RNAP/ribosome-bound states, no target trajectory forcing.",
        "non_compromise_checks": {
            "step5c_trajectory_forcing": False,
            "single_lumped_viral_mrna": False,
            "single_lumped_viral_protein": False,
            "rnapi_occupancy": f"RNAP is consumed into {prefix}_TX_locus and released on transcript completion.",
            "ribosome_occupancy": f"ribosomeP is consumed into {prefix}_RB_locus and released on protein completion.",
            "assembly_rule": "Virions are assembled by consuming one genome and one copy of each assembly-required viral protein species.",
        },
...
def install_species(sim, region_dict: dict, particles: list[dict], cds_rows: list[dict[str, str]], prefix: str) -> None:
    mobile_regions = ["cytoplasm", "outer_cytoplasm", "DNA", "ribosomes", "ribo_centers"]
    all_regions = mobile_regions + ["membrane", "extracellular"]
    handles = {}
    for name in species_names(cds_rows, prefix):
        sp = sim.species(name)
        handles[name] = sp
        diff = species_diffusion(name, prefix)
        dc = sim.diffusionConst(f"{name}_diff", diff)
        regions = all_regions if diff == 0.0 or name == f"{prefix}_released" else mobile_regions
        if name == f"{prefix}_membrane_damage" or name == f"{prefix}_breach":
            regions = ["membrane"]
        for r1 in regions:
            for r2 in regions:
...
def replication_locus(cds_rows: list[dict[str, str]]) -> str | None:
    for row in cds_rows:
        text = f"{row.get('gene', '')} {row.get('product', '')}".lower()
        if any(token in text for token in ["polymerase", "replication", "replicase"]):
            return row["locus_tag"]
    return None


def add_viral_gene_resolved_reactions(sim, params: dict[str, str], cds_rows: list[dict[str, str]], region_dict: dict, prefix: str) -> None:
    rnap = sim.species("RNAP")
    ribosome = sim.species("ribosomeP")
    genome = sim.species(f"{prefix}_Genome")
    rep_event = sim.species(f"{prefix}_rep_event")
    pol_locus = replication_locus(cds_rows)
    pol = sim.species(f"{prefix}_P_{pol_locus}") if pol_locus else None
    replication = sim.rateConst(f"{prefix}_gene_resolved_replication", 1.0e6 * float(params["replication_rate_s"]), 2) if pol else sim.rateConst(f"{prefix}_gene_resolved_replication", float(params["replication_rate_s"]), 1)
    tx_bind = sim.rateConst(f"{prefix}_gene_resolved_tx_bind", 1.0e6 * float(params["transcription_rate_s"]), 2)
    tl_bind = sim.rateConst(f"{prefix}_gene_resolved_tl_bind", 1.0e6 * float(params["translation_rate_s"]), 2)
    mrna_decay = sim.rateConst(f"{prefix}_gene_resolved_mrna_decay", 0.006, 1)
    ...
    for region in rep_regions:
        ...
        if pol:
            ...
            r.addReaction([genome], [genome, genome, rep_event], replication)
        for row in cds_rows:
            locus = row["locus_tag"]
            gene = sim.species(f"{prefix}_G_{locus}")
            tx = sim.species(f"{prefix}_TX_{locus}")
            mrna = sim.species(f"{prefix}_R_{locus}")
            rb = sim.species(f"{prefix}_RB_{locus}")
            protein = sim.species(f"{prefix}_P_{locus}")
            tx_event = sim.species(f"{prefix}_TX_{locus}_event")
            tl_event = sim.species(f"{prefix}_TL_{locus}_event")
            tx_done = sim.rateConst(f"{prefix}_TX_done_{locus}", max(0.005, 45.0 / max(1.0, float(row["nt_length"]))), 1)
            tl_done = sim.rateConst(f"{prefix}_TL_done_{locus}", max(0.005, 15.0 / max(1.0, float(row["aa_length"]))), 1)
...
def make_solver(base_cls, cds_rows: list[dict[str, str]], prefix: str):
    import Communicate as communicate  # noqa: F401

    pol_locus = replication_locus(cds_rows)
    structural_loci = [row["locus_tag"] for row in cds_rows if row["locus_tag"] != pol_locus]
    cds_by_locus = {row["locus_tag"]: row for row in cds_rows}

    class Phase7ViralGeneResolvedSolver(base_cls):
        ...
        def _apply_replication_templates(self, plattice, rep_events: int) -> None:
            for _ in range(rep_events):
                for row in cds_rows:
                    self._place_one(plattice, f"{prefix}_G_{row['locus_tag']}", "cyto")

        def _apply_gene_event_costs(self, live_t: int, plattice) -> None:
            params = self.sim_properties["phase7_viral_parameters"]
            rep = self._count_species(plattice, f"{prefix}_rep_event")
            asm = self._count_species(plattice, f"{prefix}_assembly_event")
            if rep:
            ...
            for row in cds_rows:
                locus = row["locus_tag"]
                tx_name = f"{prefix}_TX_{locus}_event"
                tl_name = f"{prefix}_TL_{locus}_event"
            ...
            for name in [f"{prefix}_rep_event", f"{prefix}_assembly_event"]:
                for _ in range(self._count_species(plattice, name)):
                    self._remove_one(plattice, name)
            for row in cds_rows:
                for suffix in ["TX", "TL"]:
                    name = f"{prefix}_{suffix}_{row['locus_tag']}_event"
                    for _ in range(self._count_species(plattice, name)):
                        self._remove_one(plattice, name)

        def _assemble_virions(self, live_t: int, plattice) -> None:
            genome = self._count_species(plattice, f"{prefix}_Genome")
            available_sets = genome
            for locus in structural_loci:
                available_sets = min(available_sets, self._count_species(plattice, f"{prefix}_P_{locus}"))
            if available_sets <= 0:
                return
            rate = float(self.sim_properties["phase7_viral_parameters"]["assembly_rate_s"])
            ...
            for _ in range(n_to_assemble):
                if not self._remove_one(plattice, f"{prefix}_Genome"):
                    break
                ok = True
                removed = []
                for locus in structural_loci:
                    species = f"{prefix}_P_{locus}"
                ...
                if not ok:
                    ...
                    self._place_one(plattice, f"{prefix}_Genome", "cyto")
                    break
                if self._place_one(plattice, f"{prefix}_virion", "cyto"):
                    self._place_one(plattice, f"{prefix}_assembly_event", "cyto")
                    made += 1
            if made:
        ...
        def _apply_membrane_release(self, live_t: int, plattice) -> None:
            virion = self._count_species(plattice, f"{prefix}_virion")
            ...
            current_damage = self._count_species(plattice, f"{prefix}_membrane_damage")
            for _ in range(max(0, self.damage_total - current_damage)):
                self._place_one(plattice, f"{prefix}_membrane_damage", "membrane")
            if self.damage_total >= threshold and self.breach_time is None:
                self.breach_time = live_t
                self._place_one(plattice, f"{prefix}_breach", "membrane")
            if self.breach_time is not None:
                release_target = (live_t - int(self.breach_time) + 1) * int(self.config["release_per_second"])
                while self.released_total < release_target and self._remove_one(plattice, f"{prefix}_virion"):
                    if self._place_one(plattice, f"{prefix}_released", "extra"):
    ...
    return Phase7ViralGeneResolvedSolver
...
def main() -> None:
    ...
    params_path = parameter_path(args.virus)
    params = read_parameter_table(params_path)
    expected_count = int(float(params["cds_count"])) if "cds_count" in params else None
    cds_rows = read_viral_cds_table(CDS_TABLE, args.virus, expected_count)
    prefix = species_prefix(args.virus)
    particles = make_initial_particles(cds_rows, int(args.initialGenomes), int(args.placementSeed), prefix)
    config = freeze_config(args, params, cds_rows, particles, params_path)
    if args.prepareOnly:
    ...
    sim_properties["phase7_viral_parameters"] = params
    sim_properties["phase7_viral_cds"] = cds_rows
    sim_properties["phase7_species_prefix"] = prefix
    ...
    install_species(sim, region_dict, particles, cds_rows, prefix)
    ...
    add_viral_gene_resolved_reactions(sim, params, cds_rows, region_dict, prefix)
    MCRDME.createParticleIdxMap(sim, sim_properties)
    ...
    import Hook
    Solver = makeSolver(IntMpdRdmeSolver, make_solver(Hook.MyOwnSolver, cds_rows, prefix))
    ...
    (run_dir / f"phase7_{args.virus.lower()}_gene_resolved_config.json").write_text(json.dumps(config, indent=2), encoding="utf-8")
